...
#evaluator.evaluate(stream=stream,model=[knn,ht,nb,wv_knn],model_names=['KNN','HoeffdingTree','NaiveBayes','Weighted_vote'])

evaluator.evaluate(stream=stream,model=[wv_knn],model_names=['Weighted_vote'])
cm = evaluator.get_mean_measurements(0).confusion_matrix
print("Recall per class")
# Recall = True Positive / (True Positive + False Negative)
for i in range(cm.n_classes):
